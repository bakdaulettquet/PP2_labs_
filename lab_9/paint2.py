...
def draw_qyral_buttons():
    ...
    #rhombus button
    pygame.draw.rect(screen, aq, pygame.Rect(450, uzyndyq - 60, 100, 50))
    pygame.draw.rect(screen, qara, pygame.Rect(450, uzyndyq - 60, 100, 50), 5)
    text = font.render("Rhombus", True, qara)
    screen.blit(text, (455, uzyndyq - 50))
...
def draw_shapes():
    ...
    elif qyral == "rhombus" and salu:
        #drawing rhombus shape
        half_width = (myshka_x - basy_x)
        half_height = (myshka_y - basy_y)
        
        #drawing of rhombus to ensure automatic direction (opposite)
        pygame.draw.polygon(screen, current_color, [
            (basy_x, basy_y),  
            (basy_x + half_width, basy_y - half_height), 
            (myshka_x, myshka_y),
            (basy_x - half_width, basy_y + half_height)
        ], 2)
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            if 10 <= event.pos[0] <= 110 and uzyndyq - 60 <= event.pos[1] <= uzyndyq - 10:
                ...
                qyral = "eraser"
            elif 450 <= event.pos[0] <= 550 and uzyndyq - 60 <= event.pos[1] <= uzyndyq - 10:
                qyral = "rhombus"
